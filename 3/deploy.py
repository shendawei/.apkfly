...
def exclude_aar_dep_source(tModule, dModules):
    print('开始部署依赖，排除%s中的%s AAR依赖，直接依赖其源码' % (tModule, ",".join(dModules)))
    ...
    print("2、includeModule的maven信息读取完毕，并在ext.deps[ ]中打开依赖")

    # 3、排除aar,依赖源码
    print("3、开始为子工程加入Dep Excludes")
    mainModuleInfo = None
    sourceModuleInfos = []
            ...
            if not (line == "" or line_.startswith('//') or line_.startswith('/') or line_.startswith('*')):
                ...
                if  start:
                    ...
                    if isMatch:
                        ...
                        matchObj = re.match(".*'((com|cn)\.gome\.[^']*)'", line, re.M|re.I)
                        if matchObj:
                            ga = matchObj.group(1)
                            gas = ga.split(':')
...
def writeConfigurationsExcludesAndCompileToBuildGradle(moule, moduleInfos):
    ...
    print("    找到build.gradle文件: %s" % moduleBuildGradle)
...
def getNum(numRang):
    try:
        ...
        if inputNum not in list(range(1, numRang + 1)):
            print('input err, num rang: (1 - %s)' % numRang)
            return getNum(numRang)
        else:
            return inputNum
...
def startApp(apkPath):
    ...
    print('如未启动app，请检查手机设置，允许app后台启动')
